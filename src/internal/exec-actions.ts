...
export function execActions<R>(subscriber: Subscriber<R>, actions: Action<any, any>[]) {
  const queue: Action<any, any>[] = [...actions];
  const data: any[] = [];
  let actionIndex = 0;

  const execAction = <AT, AR>(action: Action<AT, AR>) => {
    const prevActionValueIndex = (data.length > 0 ? data.length - 1 : undefined) as number;
    const prevActionValue: AT =
      prevActionValueIndex != null ? data[prevActionValueIndex] : undefined;

    action.exec(prevActionValue, prevActionValueIndex, data, actionIndex++).subscribe(
